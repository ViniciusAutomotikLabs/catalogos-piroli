-- FE-XX: adiciona aplicacao_resumo ao retorno da RPC buscar_produtos
-- Alterar o RETURNS TABLE exige DROP + CREATE (CREATE OR REPLACE não permite mudar o tipo de retorno).

DROP FUNCTION IF EXISTS public.buscar_produtos(TEXT, TEXT, BOOLEAN, INT, INT);
...
BEGIN
    ...
    IF v_termo IS NULL THEN
        ...
        SELECT
            ...
            p.codigo_produto_interno::TEXT,
            p.numero_produto::TEXT,
            coalesce(p.titulo_normalizado, p.descricao)::TEXT,
            coalesce(p.descricao_original, p.descricao)::TEXT,
            p.descricao::TEXT,
            ...
            coalesce(
                (SELECT array_agg(DISTINCT rc.numero_referencia::TEXT ORDER BY rc.numero_referencia::TEXT)
    ...
    filtrados AS (
        -- Alias obrigatório: "score" sem qualificação é ambíguo com a coluna de saída do RETURNS TABLE
        SELECT cand.* FROM candidatos cand WHERE cand.score > 0
    ),
    total AS (
    ...
    SELECT
        c.id,
        coalesce(c.codigo_principal, c.codigo_produto_interno)::TEXT,
        c.codigo_produto_interno::TEXT,
        c.numero_produto::TEXT,
        coalesce(c.titulo_normalizado, c.descricao)::TEXT,
        coalesce(c.descricao_original, c.descricao)::TEXT,
        c.descricao::TEXT,
        c.origem_catalogo::TEXT,
        c.foto_url::TEXT,
        c.unidade::TEXT,
        c.fabricante::TEXT,
        coalesce(
